...
/// </summary>
public static class KTX2_Bindings
{
    ...
    // unsafe public static Texture GetKTX2Texture(string ddsFilePath)
    // {
    //     Ktx2.CreateFromNamedFile(ddsFilePath, TextureCreateFlag.LoadImageData, out Texture* texture);

    //     Texture safeTexture = *texture;
    ...
    unsafe public static ScratchImage GetDDSImage(byte[] array, DDSFlags flags = DDSFlags.None)
    {
        ...
        try
        {
            ...
            DirectXTex.LoadFromDDSMemory((void*)ptr, (nuint)array.Length, flags, &metadata,ref image);
            return image;
        }
        finally
        {
    ...
    unsafe public static ImageSection[] GetDDSImageSections(ScratchImage ddsImage)
    {
        ImageSection[] section = new ImageSection[ddsImage.GetImageCount()];
