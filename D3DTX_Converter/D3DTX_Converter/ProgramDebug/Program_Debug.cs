...
namespace D3DTX_Converter.ProgramDebug
{
    public static class Program_Debug
    {
        ...
        */
        ...
        public static bool fixes_dds_to_d3dtx = true; //FIXES normal maps and swizzle channels

        public static bool mode_d3dtx_to_dds = false; //.d3dtx ---> .dds
        public static bool fixes_d3dtx_to_dds = true; //FIXES normal maps and swizzle channels

        //dds into standard images
        public static bool mode_dds_to_png = true; //.dds ---> .png
        public static bool mode_dds_to_tga = false; //.dds ---> .tga
        ...
        public static bool mode_tga_to_dds = false; //.tga ---> .dds
        ...
        {
            //main conversion to and from d3dtx
            if (mode_d3dtx_to_dds) { Program_D3DTX_TO_DDS.Execute(fixes_d3dtx_to_dds); return; }
            if (mode_dds_to_d3dtx) { Program_DDS_TO_D3DTX.Execute(fixes_dds_to_d3dtx); return; }

            //dds into standard images
            if (mode_dds_to_png) { Program_DDS_TO_PNG.Execute(); return; }
            if (mode_dds_to_tga) { Program_DDS_TO_TGA.Execute(); return; }
            if (mode_dds_to_bmp) { Program_DDS_TO_BMP.Execute(); return; }
            if (mode_dds_to_tiff) { Program_DDS_TO_TIFF.Execute(); return; }
            if (mode_dds_to_jpeg) { Program_DDS_TO_JPEG.Execute(); return; }

            //standard images into dds
            if (mode_png_to_dds) { Program_PNG_TO_DDS.Execute(); return; }
            if (mode_tga_to_dds) { Program_TGA_TO_DDS.Execute(); return; }
            if (mode_bmp_to_dds) { Program_BMP_TO_DDS.Execute(); return; }
            if (mode_tiff_to_dds) { Program_TIFF_TO_DDS.Execute(); return; }
            if (mode_jpeg_to_dds) { Program_JPEG_TO_DDS.Execute(); return; }
        }
    }
}
