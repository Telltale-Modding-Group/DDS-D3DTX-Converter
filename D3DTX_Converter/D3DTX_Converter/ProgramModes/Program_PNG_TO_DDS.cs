...
namespace D3DTX_Converter.ProgramModes
{
    public static class Program_PNG_TO_DDS
    {
        public static void Execute()
        {
            ...
            Console.WriteLine("PNG to DDS Texture Converter");

            //-----------------GET TEXTURE FOLDER PATH-----------------
            ConsoleFunctions.SetConsoleColor(ConsoleColor.DarkGray, ConsoleColor.White);
            Console.WriteLine("Enter the folder path with the PNG textures.");
        ...
        public static void ConvertBulk(string texPath, string resultPath)
        {
            ...
            //filter the array so we only get .png files
            textures = IOManagement.FilterFiles(textures, ".png");

            //if no png files were found, abort the program from going on any further (we don't have any files to convert!)
            if (textures.Count < 1)
            {
                ConsoleFunctions.SetConsoleColor(ConsoleColor.Black, ConsoleColor.Red);
                Console.WriteLine("No .png files were found, aborting."); //notify the user we found x amount of png files in the array
                Console.ResetColor();
                return;
            }

            ConsoleFunctions.SetConsoleColor(ConsoleColor.Black, ConsoleColor.Green);
            Console.WriteLine("Found {0} Textures.", textures.Count.ToString()); //notify the user we found x amount of png files in the array
        ...
        /// <summary>
        /// The main function for reading and converting said .png into a .dds file
        /// </summary>
