...
namespace D3DTX_Converter.ProgramDebug
{
    public static class Program_DDS_TO_BMP
    {
        ...
        {
            //intro message
            Console.WriteLine("DDS to BMP Texture Converter");
            ...
            Console.WriteLine("Enter the resulting path where converted BMP textures will be stored.");
        ...
        public static void ConvertBulk(string texPath, string resultPath, bool fixes_dds_to_generic)
        {
            ...
            {
                Console.WriteLine(
                    "No .dds files were found, aborting."); //notify the user we found x amount of dds files in the array
                Console.ResetColor();
                return;
            }

            Console.WriteLine("Found {0} Textures.",
                textures.Count.ToString()); //notify the user we found x amount of dds files in the array
        ...
        /// <summary>
        /// The main function for reading and converting said .dds into a .bmp file
        /// </summary>
        ...
        public static void ConvertTextureFile(string sourceFile, string destinationDirectory, bool fixes_dds_to_generic)
        {
            ...
            string outputTextureFilePath_BMP =
                destinationDirectory + @"\" + textureFileNameOnly + Main_Shared.bmpExtension;
            ...
            if (File.Exists(textureFilePath_JSON))
            {
                ...
                d3dtx_file.ReadD3DTXJSON(textureFilePath_JSON);

                //get the d3dtx texture type
                TelltaleEnums.T3TextureType d3dtxTextureType = d3dtx_file.GetTextureType();

                if (d3dtxTextureType == TelltaleEnums.T3TextureType.eTxBumpmap ||
                    d3dtxTextureType == TelltaleEnums.T3TextureType.eTxNormalMap)
                {
                    MasterOptions options = new();
                    options.outputDirectory = new() { directory = destinationDirectory };
                    options.outputOverwrite = new();
                    options.outputFileType = new() { fileType = TexconvEnums.TexconvEnumFileTypes.bmp };
                ...
                {
                    if (fixes_dds_to_generic)
                        NormalMapProcessing.FromDDS_NormalMapReconstructZ(sourceFile, outputTextureFilePath_BMP);
                    // else
                    //     NormalMapConvert.ConvertNormalMapToBMP(sourceFile);
                    //
                }
                else
                {
            ...
            if (File.Exists(outputTextureFilePath_BMP) == false)
            {
                Console.WriteLine("Failed to convert DDS image into a BMP!!!");
                Console.WriteLine("{0}", textureFileNameOnly);
            }
        }
    }
