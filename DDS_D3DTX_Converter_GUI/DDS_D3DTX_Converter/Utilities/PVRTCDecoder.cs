...
namespace Pvrtc
{
	public class PvrtcDecoder
	{
		...
		public byte[] DecompressPVRTC(byte[] input, int xDim, int yDim, bool do2bitMode)
		{
			AmtcBlock[] blocks = GenerateBlocks(input);
			byte[] output = new byte[xDim * yDim * sizeof(uint)];
			PVRDecompress(blocks, do2bitMode, xDim, yDim, output);
			return output;
		}

		public void DecompressPVRTC(byte[] input, int xDim, int yDim, byte[] output, bool do2bitMode)
		{
			AmtcBlock[] blocks = GenerateBlocks(input);
			PVRDecompress(blocks, do2bitMode, xDim, yDim, output);
		}
		...
		private void PVRDecompress(AmtcBlock[] compressedData, bool do2bitMode, int xDim, int yDim, byte[] output)
		{
			...
			for (int y = 0; y < yDim; y++)
			{
				for (int x = 0; x < xDim; x++)
				{
					...
					output[position + 0] = (byte)((m_aSig[2] * 8 + mod * (m_bSig[2] - m_aSig[2])) >> 3);
					output[position + 1] = (byte)((m_aSig[1] * 8 + mod * (m_bSig[1] - m_aSig[1])) >> 3);
					output[position + 2] = (byte)((m_aSig[0] * 8 + mod * (m_bSig[0] - m_aSig[0])) >> 3);
					output[position + 3] = doPT ? (byte)0 : (byte)((m_aSig[3] * 8 + mod * (m_bSig[3] - m_aSig[3])) >> 3);
				}
			}
		}
